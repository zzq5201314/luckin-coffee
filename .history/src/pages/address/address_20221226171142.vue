...
export default {
  ...
  methods: {
    ...
    getAddressData () {
      ...
      this.address = [provinceList, cityAllList[0], countyList[0]];
    ...
    selMonitor (e) {
      ...
      console.log("selMonitor => column", column)
      if (column == 0) {  // 滚动省
        let index = e.detail.value
        console.log("selMonitor => index", index)
        let cityLength = that.address[1].length
        console.log("selMonitor => length", cityLength)

        let countyLength = that.address[2].length
        console.log("selMonitor => countyLength", countyLength)
        this.getAddressData()
        // 改变市
        that.address[1].splice(0, cityLength, ...that.cityAllList[index])

        that.address[2].splice(0, countyLength, ...that.countyList[index])
      }
