 ...
 * @LastEditTime: 2022-12-27 18:05:00
    ...
    >
      ...
      >
        ...
        >
          ...
          <view class="pb-4 text-right">
            <view v-show="productItem.status==1">订单进行中</view>
            <view v-show="productItem.status==2">订单已完成</view>
            <!-- {{productItem.status==1?'订单进行中':''}}
            {{productItem.status==2?'订单已完成':''}} -->
          </view>

          <view
            class="flex  items-center gap-2 border-0 border-gray-200 border-b border-dotted pb-4"
          >
            <image
...
<script>
import { findOrder, receive } from "@/api/order"
export default {
  name: "order",
  data () {
    return {
