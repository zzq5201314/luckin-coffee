 ...
 * @LastEditTime: 2022-12-27 23:48:16
...
export default {
  ...
  onUnload () { // 监听页面卸载
    ...
    if (this.paySid) {
      this.$store.dispatch("pay/resetState", [this.paySid]).then(_ => {

        console.log('执行获取购物车数据');
    ...
    pay () {
      ...
      pay(data).then(response => {
        ...
        if (response.code === 60000) {
          ...
          this.$store.dispatch('shopCart/removeShopCart', this.$Route.query.sids).then(() => {
            // 更新购物车数据
            this.$store.dispatch('shopCart/getShopCartData')
