 ...
 * @LastEditTime: 2022-12-29 17:19:37
  ...
  >
    ...
    >
      ...
      <view
        ...
        :key="index"
        :data-index="index"
        class="flex items-center py-3 mx-3 border-0 border-b border-solid border-gray-100 gap-4 relative overflow-hidden overflow-x-scroll"
      ...
      >
        ...
        <view
          class="remove absolute top-0 right-0 h-full text-white bg-red-600 rounded-r-lg -mr-3 flex justify-center items-center "
        >删除
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    drawMove (e) {
      ...
      // }
      // console.log("drawMove => e.currentTarget.dataset.index", e.currentTarget.dataset.index)
    ...
    drawEnd (e) {
      console.log('滑动结束');
      // let item = this.addressList[e.currentTarget.dataset.index];
      // // 滑动结束时 滑动的值与 设置的值做判断
      // if (item.right >= this.delBtnWidth / 2) {
      //   // 动态设置 属性
      //   this.$set(this.addressList[e.currentTarget.dataset.index], 'right', this.delBtnWidth);
      //   this.$set(this.addressList[e.currentTarget.dataset.index], 'opacity', 1);
      // } else {
      //   this.$set(this.addressList[e.currentTarget.dataset.index], 'right', 0);
      //   this.$set(this.addressList[e.currentTarget.dataset.index], 'opacity', 0);
