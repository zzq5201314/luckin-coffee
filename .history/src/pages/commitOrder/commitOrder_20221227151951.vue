 ...
 * @LastEditTime: 2022-12-27 15:19:16
...
<template>
  <view class="commitOrder bg-bgColor p-3">
    ...
    <view class="flex flex-col gap-2 bg-white mt-3 p-4">
      ...
      getCommitOrderProductData(this.$Route.query.sids).then(response => {
        if (response.code === 50000) {
          ...
          response.result.forEach(item => {
            ...
            this.price = this.price + parseFloat(item.price) * item.count

            item.price = parseFloat(item.price)

          })

          console.log("getCommitOrderProductData => this.price", this.price)
